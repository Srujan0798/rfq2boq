...
def detect_scope_gaps(entities: list[dict], relations: list[dict]) -> list[str]:
    ...
    for material in material_entities:
        ...
        has_quantity = any(
            r.get("type") == "HAS_QUANTITY" and r.get("head_id") == material_id
            for r in relations
        )

        if not has_quantity:
            warnings.append(
                f"SCOPE_GAP: Material '{material.get('text', '')}' has no quantity"
            )

        has_unit = any(
            r.get("type") == "AT_LOCATION" and r.get("head_id") == material_id
            or r.get("type") == "OF_GRADE" and r.get("head_id") == material_id
            or r.get("type") == "COMPLIES_WITH" and r.get("head_id") == material_id
            for r in relations
        )
    ...
    for quantity in quantity_entities:
        ...
        has_unit = any(
            r.get("type") == "HAS_UNIT" and r.get("head_id") == qty_id
            for r in relations
        )

        if not has_unit:
            warnings.append(
                f"SCOPE_GAP: Quantity '{quantity.get('text', '')}' has no unit"
            )
