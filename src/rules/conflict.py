...
def _resolve_group(group: list[EntityCandidate]) -> EntityCandidate:
    ...
    if len(entity_types) == 1:
        ...
        strategy = STRATEGY_MAP.get(
            max(entity_types, key=lambda t: _type_priority(t)),
...
def _type_priority(t: str) -> int:
    ...
    priority = {"QUANTITY": 7, "UNIT": 7, "STANDARD": 7,
                "MATERIAL": 5, "LOCATION": 5, "ACTION": 5,
                "DIMENSION": 3, "GRADE": 3}
...
def resolve_conflicts_legacy(
    ...
    for bert_ent in bert_entities:
        ...
        if ent_type in pattern_wins:
            matching = [
                (i, p) for i, p in enumerate(pattern_entities)
    ...
    for i, pattern_ent in enumerate(pattern_entities):
        ...
        if ent_type in confidence_wins:
            overlap_berts = [b for b in bert_entities if b.get("type") == ent_type and
                       _overlaps_spans(EntityCandidate(pattern_ent), EntityCandidate(b))]
            if overlap_berts:
...
def _merge_overlapping(entities: list[dict]) -> list[dict]:
    ...
    for entity in sorted_entities:
        ...
        if (_overlaps(current, entity) and
            current.get("type") == entity.get("type") and
            entity.get("source") == current.get("source")):
