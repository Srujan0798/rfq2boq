...
def load_ner_model(model_dir: str) -> tuple[Any, AutoTokenizer]:
    ...
    try:
        ...
        model = AutoModelForTokenClassification.from_pretrained("bert-base-cased", num_labels=41)
    model.eval()
        ...
        with pdfplumber.open(str(pdf_path)) as pdf:
            ...
            for page_num, page in enumerate(pdf.pages, start=1):
                ...
                for _sent_idx, sentence in enumerate(sentences[:50]):
                    ...
                    entities = extract_entities_with_uncertainty(sentence.strip(), model, tokenizer, id2label)
...
ID2LABEL = {
    ...
    for i, label in enumerate(
        ...
        + [
            f"{p}-{t}"
            for t in ["MATERIAL", "QUANTITY", "UNIT", "LOCATION", "DIMENSION", "STANDARD", "ACTION", "GRADE"]
            for p in ["B", "I", "E", "S"]
        ]
    )
}
