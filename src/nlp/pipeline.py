...
class NLPPipeline:
    ...
    def _init_ner(self, model_dir: str | None) -> None:
        ...
        final_model_dir = base_path / "models/rfq2boq-ner-final/final_model"
        fallback_model_dir = base_path / "models/ner-bert-bilstm-crf-v1"

        if model_dir:
            model_path = Path(model_dir)
            if model_path.exists() and (model_path / "config.json").exists():
                self._load_lazy_ner(str(model_path))
            return

        if (final_model_dir / "config.json").exists():
            self._load_lazy_ner(str(final_model_dir))
        elif (fallback_model_dir / "config.json").exists():
            self._load_lazy_ner(str(fallback_model_dir))
    ...
    def _load_lazy_ner(self, model_path: str) -> None:
        """Load NER model using lazy approach to avoid import hangs."""
        try:
            from src.nlp.ner.lazy_model import get_ner_model
            self.ner = get_ner_model(model_path)
        except Exception:
            self.ner = None
    ...
    def process(
        ...
        avg_confidence = (
            sum(e.get("confidence", 0) for e in merged_entities) / len(merged_entities)
            if merged_entities else 0.0
        )
