"""ARCBERT (SciBERT fallback) BiLSTM-CRF NER model."""
...
class SciBERTBiLSTMCRF(nn.Module):
    def __init__(
        self,
        model_name: str = "models/arcbert-base",
        num_labels: int = 41,
        lstm_hidden: int = 256,
        lstm_layers: int = 1,
        dropout: float = 0.1,
...
class SciBERTNER:
    ...
    def _decode_predictions(self, tokens: list[str], predictions: torch.Tensor, scores: torch.Tensor) -> list[dict]:
        entities = []
        current_entity = None
        ...
        for i, (token, pred_id, score) in enumerate(zip(tokens, predictions, scores, strict=False)):
            ...
            if label.startswith("S-"):
                ...
                if label[2:] == current_entity["type"]:
                    ...
                    current_entity["confidence"] = max(current_entity["confidence"], score[pred_id].item())
                else:
                    entities.append(current_entity)
                    current_entity = None
...
def load_pretrained_scibert_ner(
    model_dir: str,
    model_name: str = "models/arcbert-base",
    num_labels: int = 41,
    id2label: dict[int, str] | None = None,
