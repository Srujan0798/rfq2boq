...
class ConstructionOntology:
    ...
    def _load(self) -> None:
        ...
        # Load insulation ontology from data/ontology directory
        from pathlib import Path

        insulation_dir = Path.cwd() / "data" / "ontology"
        self._insulation_materials = self._load_json_from_dir(insulation_dir, "insulation_materials.json", "materials")
        self._insulation_standards = self._load_json_from_dir(insulation_dir, "insulation_standards.json", "standards")
        self._insulation_units = self._load_json_from_dir(insulation_dir, "insulation_units.json", "units")
    ...
    def _load_json_from_dir(self, directory: Path, filename: str, key: str) -> dict[str, dict]:
        result = {}
    ...
    def get_default_unit(self, material: str) -> str:
        ...
        if mat_entry and mat_entry.get("common_units"):
            return str(mat_entry["common_units"][0])
        return "nos"

    def resolve_unit_symbol(self, unit_text: str) -> str:
        entry = self._units.get(unit_text.lower().strip())
        if entry:
            return str(entry.get("symbol", unit_text.strip()))
    ...
    @property
    def insulation_materials(self) -> dict[str, dict]:
        return self._insulation_materials

    @property
    def insulation_standards(self) -> dict[str, dict]:
        return self._insulation_standards

    @property
    def insulation_units(self) -> dict[str, dict]:
        return self._insulation_units

    def find_compatible_standards(self, material: str) -> list[dict]:
        """Return standards whose applies_to list mentions this material."""
        mat_lower = material.lower()
        return [
            std
            for std in self._standards.values()
            if any(at in mat_lower or mat_lower in at for at in std.get("applies_to", []))
        ]

    def convert_unit(self, value: float, from_unit: str, to_unit: str) -> float | None:
        """Best-effort unit conversion; returns None when no factor is known."""
        from_unit = from_unit.lower().strip()
        to_unit = to_unit.lower().strip()
        if from_unit == to_unit:
            return value
        # Simple factor table for common construction units.
    ...
    def lookup_insulation_material(self, name: str) -> dict | None:
        """Look up an insulation material by name or alias."""
        key = name.lower().strip()
        return self._insulation_materials.get(key)

    def lookup_insulation_standard(self, code: str) -> dict | None:
        """Look up an insulation standard by code or alias."""
        key = code.lower().strip()
        return self._insulation_standards.get(key)

    def get_all_insulation_materials(self) -> list[dict]:
        """Get all insulation materials."""
        seen = set()
        result = []
        for entry in self._insulation_materials.values():
            name = entry.get("name", "")
            if name and name not in seen:
                seen.add(name)
                result.append(entry)
    ...
    def get_all_insulation_standards(self) -> list[dict]:
        """Get all insulation standards."""
        seen = set()
        result = []
        for entry in self._insulation_standards.values():
            code = entry.get("code", "")
            if code and code not in seen:
                seen.add(code)
                result.append(entry)
