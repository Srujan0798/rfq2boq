...
Unit normalization is delegated to src.rules.units (single source of truth).
This module retains dimension-specific helpers plus a thin ``normalize_unit``
wrapper for backwards compatibility with call sites that historically
...
def parse_dimension(text: str) -> dict[str, float] | None:
    ...
    compound = re.match(
        r"(?P<length>\d+(?:\.\d+)?)\s*(?P<lu>mm|cm|m)\s*[x×]\s*" r"(?P<width>\d+(?:\.\d+)?)\s*(?P<wu>mm|cm|m)\b",
        value,
        flags=re.IGNORECASE,
    )
    if compound:
...
def calculate_quantity(*args: Any) -> float | tuple[float, str]:
    ...
    """
    if len(args) >= 3:
        quantity = to_float_qty(args[1]) or 0.0
        return quantity, str(args[2])
    if not args:
        return 0.0
    return to_float_qty(args[0]) or 0.0
