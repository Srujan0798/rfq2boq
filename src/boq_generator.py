...
class BOQGenerator:
    ...
    def generate(self, items_or_result: ExtractionResult | list[BoqRow] | list[dict[str, Any]]) -> dict[str, Any]:
        if isinstance(items_or_result, ExtractionResult):
            ...
            items: list[BoqRow | dict[str, Any]] = list(items_or_result)  # type: ignore[arg-type]
            rows = [self._coerce_row(item, index) for index, item in enumerate(items, start=1)]
    ...
    def _format_item(self, item: BoqRow) -> dict[str, Any]:
        ...
        # Sequential ints keep BOQ-001 style codes; hierarchical codes pass through.
        item_code = (
            f"BOQ-{item.item_no:03d}" if isinstance(item.item_no, int) else f"BOQ-{item.item_no}"
        )
        return {
            "item_code": item_code,
    ...
    def _coerce_row(self, item: BoqRow | dict[str, Any], index: int) -> BoqRow:
        if isinstance(item, BoqRow):
            if isinstance(item.item_no, int) and item.item_no <= 0:
                item.item_no = index
            return item

        raw_no = item.get("item_no")
        if raw_no is None or raw_no == "" or raw_no == 0:
            raw_no = index
