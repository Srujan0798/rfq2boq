...
class BoqRiskAnalyzer:
    ...
    def _load_norms(self, norms_path: str | Path) -> None:
        try:
    ...
    def _price_outlier(self, item: BoqRow) -> float:
        # Pricing outlier removed (unpriced BOQ scope - S1_STRIP_RATES)
        return 0.0
    ...
    def _missing_standard(self, item: BoqRow) -> float:
        ...
        mat_is_construction = (
            any(mat in item.material.lower() for mat in construction_materials) if item.material else False
        )
