...
class MissingStandardFactor:
    ...
    def calculate(self, item: Any) -> RiskFactorResult:
        ...
        is_construction = any(
            mat in item.material.lower()
            for mat in self.CONSTRUCTION_MATERIALS
        ) if item.material else False
        ...
        has_is_ref = any(
            s in str(item.standard or "").lower()
            for s in self.IS_STANDARDS
        ) if item.standard else False
...
class LowConfidenceFactor:
    def calculate(self, item: Any, threshold: float = 0.5) -> RiskFactorResult:
        ...
        return RiskFactorResult(name="low_confidence", score=score, details={"confidence": conf, "threshold": threshold})


class UnknownMaterialFactor:
    KNOWN_MATERIALS = {
        "cement", "concrete", "steel", "sand", "aggregate", "brick", "block",
        "paint", "tile", "wood", "plywood", "glass", "pipe", "cable",
        "aluminum", "copper", "gi", "pvc", "cpvc", "upvc", "tmt", "bitumen",
    }
