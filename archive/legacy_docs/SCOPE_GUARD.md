...
| Folder | Status |
|--------|--------|
| `prompts/archive/hybrid/phase1/` | Historical (P1T1–P1T5; see current wave4 or unified for active) |
| `prompts/archive/hybrid/phase2/` | Historical (slim codebase; superseded) |
| `prompts/archive/hybrid/phase3/` | Historical (final polish; superseded) |
| `prompts/archive/wave2/` | Historical (A0, A3 etc.; superseded) |
| `prompts/archive/wave3/` | Historical (B1, B2; superseded) |
| `prompts/wave4/` | Active (current wave4 agents B1-F4, C, D, E) |
| `prompts/archive/out_of_scope/` | **READ-ONLY** — never dispatch from here |

Old waves (wave2/3, hybrid/) are now under `prompts/archive/`. Active dispatch is from `prompts/wave4/`. Out-of-scope prompts under `archive/` exist for traceability. Do not dispatch them; do not auto-port them into active folders.
