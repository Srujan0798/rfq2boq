...
class TestAmbiguityResolver:
    ...
    async def test_resolve_low_confidence_calls_llm(self):
        from src.llm.client import AmbiguityResolver
        resolver = AmbiguityResolver()
        resolver.llm.query = AsyncMock(return_value=MagicMock(
            text="MATERIAL", model="claude", usage={}, cached=False
        ))
...
class TestSummaryGenerator:
    ...
    async def test_generate_summary_calls_llm(self):
        from src.llm.client import SummaryGenerator
        gen = SummaryGenerator()
        gen.llm.query = AsyncMock(return_value=MagicMock(
            text="Executive summary of BOQ.", model="claude", usage={}, cached=False
        ))
...
class TestBOQQA:
    ...
    async def test_ask(self):
        from src.llm.client import BOQQA
        qa = BOQQA()
        qa.llm.query = AsyncMock(return_value=MagicMock(
            text="Total cement is 500 kg.", model="claude", usage={}, cached=False
        ))
