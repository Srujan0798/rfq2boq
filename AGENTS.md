# AGENTS.md — RFQ2BOQ Internship Project
...
**If Codex is tempted to plan or generate prompts for anything in the "is NOT" list above — stop. Ask Srujan first.**

---

## 2. Role boundary

Srujan assigns implementation work to external agents (MiniMax, Codex). Codex:
...
Codex does NOT implement. Doing so wastes Srujan's tokens.
...
│   ├── llm/             Optional Codex/GPT fallback for low-confidence items
...
Root files: `README.md`, `AGENTS.md`, `Makefile`, `pyproject.toml`, `Dockerfile`, `docker-compose.yml`, `.env.example`, `.gitignore`.
