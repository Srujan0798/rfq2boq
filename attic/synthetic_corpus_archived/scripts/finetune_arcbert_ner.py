#!/usr/bin/env python3
"""Fine-tune SciBERT (ARCBERT fallback) on BIOES annotations."""
...
def main():
    ...
    print("=" * 60)
    print("SciBERT NER Fine-tuning (ARCBERT fallback)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("STEP 2: Loading SciBERT model and tokenizer")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("STEP 3: Training (10 epochs, lr=2e-5, batch=16)")
    print("=" * 60)

    output_dir = base_path / "models/arcbert-ner-v1"
    ...
    print("\n" + "=" * 60)
    print("STEP 6: Evaluating on real RFQ annotations")
    print("=" * 60)
    ...
    with open(results_dir / "arcbert_ner_eval.json", "w") as f:
        json.dump({**metrics, **real_metrics}, f, indent=2)
    print(f"\nSaved evaluation results to {results_dir / 'arcbert_ner_eval.json'}")

    print("\n" + "=" * 60)
    print("STEP 7: Comparing with baseline")
    print("=" * 60)

    baseline_path = base_path / "models/ner-bert-bilstm-crf-v1/metrics.json"
    baseline_final_path = base_path / "models/rfq2boq-ner-final/metrics.json"

    baseline_f1 = None
    if baseline_path.exists():
        with open(baseline_path) as f:
            bm = json.load(f)
            baseline_f1 = bm.get("test_f1", bm.get("real_test_f1"))
            print(f"Baseline (ner-bert-bilstm-crf-v1) F1: {baseline_f1:.4f}" if baseline_f1 else "Baseline metrics not found")

    baseline_final_f1 = None
    if baseline_final_path.exists():
        with open(baseline_final_path) as f:
            bfm = json.load(f)
            baseline_final_f1 = bfm.get("test_f1", bfm.get("real_test_f1"))
            print(f"Final model (rfq2boq-ner-final) F1: {baseline_final_f1:.4f}" if baseline_final_f1 else "Final model metrics not found")

    comparison = {
        "arcbert_f1": micro_f1,
        "arcbert_test_f1": eval_results["eval_f1"],
        "baseline_f1": baseline_f1,
        "baseline_final_f1": baseline_final_f1,
        "improvement_vs_bert": micro_f1 - baseline_f1 if baseline_f1 else None,
        "improvement_vs_final": micro_f1 - baseline_final_f1 if baseline_final_f1 else None,
        "entities": {
            entity: {
                "arcbert_f1": real_per_entity.get(f"f1_{entity}", 0.0),
                "arcbert_precision": real_per_entity.get(f"precision_{entity}", 0.0),
                "arcbert_recall": real_per_entity.get(f"recall_{entity}", 0.0),
            }
            for entity in entities
        },
    }

    with open(results_dir / "arcbert_vs_baseline.json", "w") as f:
        json.dump(comparison, f, indent=2)
    print(f"Saved comparison to {results_dir / 'arcbert_vs_baseline.json'}")

    print("\n" + "=" * 60)
    print("FINAL SUMMARY")
    print("=" * 60)
    print(f"SciBERT (ARCBERT fallback) test F1:   {eval_results['eval_f1']:.4f}")
    print(f"SciBERT (ARCBERT fallback) real F1:   {micro_f1:.4f}")
    if baseline_f1:
        print(f"Baseline BERT real F1:               {baseline_f1:.4f}")
        print(f"Improvement:                         {micro_f1 - baseline_f1:+.4f}")
    if baseline_final_f1:
        print(f"Final model real F1:                {baseline_final_f1:.4f}")
        print(f"Improvement vs final:                {micro_f1 - baseline_final_f1:+.4f}")
