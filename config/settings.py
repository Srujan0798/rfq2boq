...
class Settings(BaseSettings):
    ...
    # Component confidence scores (previously hardcoded; now configurable)
    # RFQ2BOQ_XLSX_DIRECT_QUANTITY_CONFIDENCE: Confidence for XLSX rows with a direct positive quantity
    RFQ2BOQ_XLSX_DIRECT_QUANTITY_CONFIDENCE: float = 0.95
    # RFQ2BOQ_XLSX_RECOVERED_QUANTITY_CONFIDENCE: Confidence for XLSX rows recovered from rate-only / fallback paths
    RFQ2BOQ_XLSX_RECOVERED_QUANTITY_CONFIDENCE: float = 0.70
    # RFQ2BOQ_PDF_GEM_RECOVERY_CONFIDENCE: Confidence for PDF GeM per-item recovery rows
    RFQ2BOQ_PDF_GEM_RECOVERY_CONFIDENCE: float = 0.80
    # RFQ2BOQ_CATALOG_EXACT_MATCH_CONFIDENCE: Confidence for exact catalog alias matches
    RFQ2BOQ_CATALOG_EXACT_MATCH_CONFIDENCE: float = 0.98
    # RFQ2BOQ_TABLE_EXTRACTOR_BASE_CONFIDENCE: Base confidence for pdfplumber fallback table extraction
    RFQ2BOQ_TABLE_EXTRACTOR_BASE_CONFIDENCE: float = 0.70
    # RFQ2BOQ_TABLE_EXTRACTOR_HIGH_CONFIDENCE: High confidence for position-aware split-quantity table extraction
    RFQ2BOQ_TABLE_EXTRACTOR_HIGH_CONFIDENCE: float = 0.85
    ...
    # OpenRouter settings
    # RFQ2BOQ_OPENROUTER_API_KEY: OpenRouter API key (get from openrouter.ai)
    RFQ2BOQ_OPENROUTER_API_KEY: str = ""
    # RFQ2BOQ_OPENROUTER_MODEL: Default model to use (free models end with :free)
    RFQ2BOQ_OPENROUTER_MODEL: str = "openrouter/free"
    # RFQ2BOQ_OPENROUTER_BASE_URL: OpenRouter API base URL
    RFQ2BOQ_OPENROUTER_BASE_URL: str = "https://openrouter.ai/api/v1"

    # Pretrained NER signal (generic HuggingFace model, NOT domain-trained)
    # RFQ2BOQ_PRETRAINED_NER_ENABLED: Enable the additional pretrained NER signal
    RFQ2BOQ_PRETRAINED_NER_ENABLED: bool = True
    ...
    @property
    def XLSX_DIRECT_QUANTITY_CONFIDENCE(self) -> float:
        return self.RFQ2BOQ_XLSX_DIRECT_QUANTITY_CONFIDENCE

    @property
    def XLSX_RECOVERED_QUANTITY_CONFIDENCE(self) -> float:
        return self.RFQ2BOQ_XLSX_RECOVERED_QUANTITY_CONFIDENCE

    @property
    def PDF_GEM_RECOVERY_CONFIDENCE(self) -> float:
        return self.RFQ2BOQ_PDF_GEM_RECOVERY_CONFIDENCE

    @property
    def CATALOG_EXACT_MATCH_CONFIDENCE(self) -> float:
        return self.RFQ2BOQ_CATALOG_EXACT_MATCH_CONFIDENCE

    @property
    def TABLE_EXTRACTOR_BASE_CONFIDENCE(self) -> float:
        return self.RFQ2BOQ_TABLE_EXTRACTOR_BASE_CONFIDENCE

    @property
    def TABLE_EXTRACTOR_HIGH_CONFIDENCE(self) -> float:
        return self.RFQ2BOQ_TABLE_EXTRACTOR_HIGH_CONFIDENCE
    ...
    def OPENROUTER_API_KEY(self) -> str:
        return self.RFQ2BOQ_OPENROUTER_API_KEY

    @property
    def OPENROUTER_MODEL(self) -> str:
        return self.RFQ2BOQ_OPENROUTER_MODEL

    @property
    def OPENROUTER_BASE_URL(self) -> str:
        return self.RFQ2BOQ_OPENROUTER_BASE_URL
    ...
    @property
    def PRETRAINED_NER_ENABLED(self) -> bool:
        return self.RFQ2BOQ_PRETRAINED_NER_ENABLED
